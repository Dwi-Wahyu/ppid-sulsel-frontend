...
<li @click="if(!sidebarOpen) sidebarOpen = true">
    <a href="{{ $href }}" {{ $attributes->except('class') }}
        ...
        :class="sidebarOpen ? 'px-4 justify-start' : 'px-0 justify-center h-12 w-12 mx-auto'" @if($active)
        style="view-transition-name: active-sidebar-indicator;" @endif>
        ...
        <span class="relative z-10 transition-opacity duration-300 delay-100" x-show="sidebarOpen"
        ...
        @if($badge)
            <span
                class="ml-auto bg-ppid-accent text-ppid-primary py-0.5 px-2 rounded-full text-[10px] font-bold relative z-10"
                x-show="sidebarOpen">
                {{ $badge }}
            </span>
        @endif
