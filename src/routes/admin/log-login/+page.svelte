<script lang="ts">
	...
	function handleSearch(event: Event) {
		...
		const form = event.currentTarget as HTMLFormElement; // Mengambil elemen form
		const formData = new FormData(form); // Membuat objek FormData

		const search = formData.get('search') as string; // Ambil nilai search
		const ip = formData.get('ip') as string; // Ambil nilai IP

		// Update URL dengan parameter pencarian dan IP, serta reset ke page 1
		updateFilters({
			search,
			ip,
			page: '1'
	...
	function resetFilters() {
		goto('/admin/log-login');
	}

	// Check if has active filters
	const hasActiveFilters = $derived(
		data.filters.search || data.filters.ip || data.filters.start_date || data.filters.end_date
	);
</script>
	...
	>
		...
		>
			<div class="flex w-full flex-1 flex-col gap-3 lg:flex-row">
				<form onsubmit={handleSearch} class="flex w-full flex-1 flex-col gap-3 md:flex-row">
					...
					<div class="relative flex-1">
						<input
							...
							placeholder="Filter IP (Contoh: 127.0.0.1)"
							class="w-full rounded-lg border border-slate-200 bg-white py-2 pr-4 pl-10 text-sm text-slate-900 placeholder-slate-400 focus:border-ppid-accent focus:ring-1 focus:ring-ppid-accent focus:outline-none dark:border-slate-600 dark:bg-slate-700 dark:text-slate-100 dark:placeholder-slate-500"
						/>
						<div
					...
					<button
						type="submit"
						class="hover:bg-ppid-dark flex items-center justify-center gap-2 rounded-lg bg-ppid-primary px-4 py-2 text-sm font-medium text-white transition-colors"
					>
						<svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
							<path
								stroke-linecap="round"
					...
					<div>
						<label class="block text-xs font-medium text-slate-700 dark:text-slate-300"
							...
							<input
								...
								class="mt-2 w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700 focus:border-ppid-accent focus:outline-none dark:border-slate-600 dark:bg-slate-700 dark:text-slate-300"
							/>
						</label>
					</div>
					...
					{#if data.data?.data}
						{#each data.data.data as info}
							...
							>
								...
								<td class="px-6 py-4">
									<span class="text-xs">{info.ip}</span>
								</td>
								<td class="px-6 py-4">
									<span class="text-xs">{formatDate(info.createdAt)}</span>
								</td>
								<td class="px-6 py-4"
									>{#if info.tipe === 'login'}
										<span>Masuk</span>
			...
			<div class="border-t border-slate-100 p-4 dark:border-slate-700">
				<div class="flex items-center justify-between">
					<div class="text-sm text-slate-600 dark:text-slate-400">
						Menampilkan {data.data.from} - {data.data.to} dari {data.data.total} data
					</div>
