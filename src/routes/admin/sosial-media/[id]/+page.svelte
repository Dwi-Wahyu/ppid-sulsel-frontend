<script lang="ts">
	...
	onMount(async () => {
		try {
			const result = await api.get(`/admin/sosmed/${id}`);
			if (result.success) {
			...
			notificationMessage = 'Gagal memuat data';
	...
	async function handleSubmit(e: Event) {
		...
		try {
			const result = await api.put(`/admin/sosmed/${id}`, formData);
			if (result.success) {
				notificationType = 'success';
				notificationMessage = 'Tautan berhasil diperbarui';
				showNotification = true;
				setTimeout(() => {
