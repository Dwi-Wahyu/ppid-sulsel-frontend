<script lang="ts">
	...
	async function handleDelete() {
		...
		try {
			...
			const response = await fetch(`${PUBLIC_API_URL}/admin/pengajuan-keberatan/${deleteId}`, {
				method: 'DELETE',
				headers: {
			...
			if (result.success) {
				...
				notificationDescription = 'Pengajuan keberatan berhasil dihapus.';
				...
				notificationDescription = result.message || 'Terjadi kesalahan saat menghapus pengajuan.';
				showNotification = true;
			}
		} catch (error) {
			notificationTheme = 'error';
			notificationTitle = 'Gagal!';
			notificationDescription = 'Terjadi kesalahan saat menghapus pengajuan.';
			showNotification = true;
		}
