<script lang="ts">
	...
	let informasiData = $state<KategoriResponse | null>(null);
	...
	async function fetchData() {
		...
		try {
			...
			const response = await api.get('/public/informasi/kategori/daftar-informasi-publik', {
				page: currentPage,
	...
	let category = $derived(informasiData?.category);
	...
	<div class="container mx-auto px-4 py-6">
		<nav class="mb-4 flex items-center gap-2 text-sm text-gray-600 dark:text-gray-300">
			...
			<span class="font-medium text-ppid-primary dark:text-white">
				{category?.name ?? m['public_info.title']()}
			</span>
		</nav>
