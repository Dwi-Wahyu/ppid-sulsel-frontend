	...
	async function handleUpdate() {
		...
		try {
			...
			if (response.ok && result.success) {
				showNotification('success', result.message || 'Profil SKPD berhasil diperbarui');
				await invalidateAll();
			} else {
				if (result.errors) {
					errors = result.errors;
				}
				showNotification('error', result.message || 'Gagal memperbarui profil SKPD');
			}
		} catch (error) {
...
<svelte:head>
	<title>Profil SKPD - OPD PPID</title>
</svelte:head>
...
{:else}
	<div
		class="rounded-xl border border-slate-100 bg-white p-12 text-center shadow-sm dark:border-slate-700 dark:bg-slate-800"
	>
		<svg
		...
		<p class="mt-4 text-slate-600 dark:text-slate-400">Data SKPD tidak ditemukan</p>
	</div>
{/if}
