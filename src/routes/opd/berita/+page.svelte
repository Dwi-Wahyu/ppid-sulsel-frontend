<script lang="ts">
	...
	// User dari layout server (locals.user)
	...
	/**
	 * Mengambil data berita milik SKPD saat ini
	 */
	async function fetchData(page: number | string = 1) {
		loading = true;
		try {
			// Filter otomatis berdasarkan SKPD user yang sedang login
			const currentSkpdId = user?.id_skpd ?? null;

			const res = await api.get('/admin/berita', {
				search: search || null,
				page: page,
				...(currentSkpdId ? { id_skpd: currentSkpdId } : {})
			});
	...
	async function handleDelete() {
		...
		try {
			...
			if (res.success) {
				...
				notificationMessage = 'Berita berhasil dihapus.';
				showNotification = true;
			}
		} catch (error: any) {
			console.error('Delete error:', error);
			notificationType = 'error';
			notificationMessage = error.message || 'Gagal menghapus berita.';
			showNotification = true;
		} finally {
			isDeleting = false;
			deleteId = null;
...
<svelte:head>
	<title>Manajemen Berita - OPD</title>
</svelte:head>
...
<div class="space-y-6 p-6">
	<header class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
		<div>
			...
			<p class="text-sm font-medium text-slate-500">Kelola dan publikasi berita.</p>
		</div>
							...
							<tr class="transition-colors hover:bg-slate-50/50 dark:hover:bg-slate-700/30">
								...
								<td class="px-6 py-5 text-right">
									<div class="flex justify-end gap-2">
										<a
											href="/opd/berita/{item.id_berita}/edit"
