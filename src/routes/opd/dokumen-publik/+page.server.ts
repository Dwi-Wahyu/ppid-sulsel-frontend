...
import type { DokumenPublikApiResponse, KategoriInformasi, Skpd } from '$lib/types/DokumenPublikTypes';
...
export const load: PageServerLoad = async ({ fetch, cookies, url }) => {
    ...
    try {
        ...
        // Fetch kategori list for filter (cached public API)
        const kategoriResponse = await fetch(`${API_URL}/public/informasi/kategori`, {
            headers: {
                Accept: 'application/json'
            }
        });
        ...
        if (kategoriResponse.ok) {
            ...
            kategoriList = Array.isArray(kategoriData) ? kategoriData : (kategoriData.data || []);
        }
