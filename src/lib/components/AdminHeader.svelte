<script lang="ts">
	...
	async function fetchNotifications() {
		try {
			const res = await fetch(`${env.PUBLIC_API_URL}/admin/notifications`);
			const result = await res.json();
			if (result.success) {
				notifications = result.data.data.slice(0, 5); // Ambil 5 terbaru untuk dropdown
	...
	onMount(() => {
		fetchNotifications();
		// Polling sederhana setiap 2 menit
...
>
	<button
		...
		class="mr-4 rounded-lg p-2 text-slate-600 outline-hidden hover:bg-slate-100 focus:ring-2 focus:ring-blue-500 lg:hidden dark:text-slate-400 dark:hover:bg-slate-800"
	...
	<div class="ml-auto flex items-center gap-2 md:gap-4">
						...
						{#each notifications as notif}
							...
							>
								...
								<span class="mt-2 block text-[10px] text-slate-400"
									>{new Date(notif.created_at).toLocaleDateString()}</span
								>
							</a>
						{:else}
		...
		<div class="relative">
			<button
				...
				class="flex items-center gap-2 rounded-full p-1 pr-3 transition-colors hover:bg-slate-50 dark:border-slate-700 dark:hover:bg-slate-800"
			...
			>
				...
				<div class="">
					<div
						class="hidden text-start text-sm font-semibold text-slate-700 md:block dark:text-slate-200"
					>
						{user?.name || 'Admin'}
					</div>
					<div class="hidden text-xs text-slate-500 md:block dark:text-slate-400">
						...
						>
							<svg class="mr-3 h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"
								><path
									...
									stroke-width="2"
								/><circle cx="12" cy="12" r="3" stroke-width="2" /></svg
							>
							Pengaturan
						</a>
